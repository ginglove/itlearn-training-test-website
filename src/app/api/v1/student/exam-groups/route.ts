  ...
  try {
    ...
    const rows = await db
      .select({
        ...
        // Taken = all attempts (submitted + pending + cancelled)
        ...
        // Pending = saved & exited, close date not yet passed
        totalPending: sql<number>`COUNT(CASE
          WHEN ${examSubmissions.submittedAt} IS NULL
            AND ${examSubmissions.closeReason} = 'SAVE_AND_EXIT'
            AND ${exams.endTime} > NOW()
          THEN 1 END)`,
        // Cancelled = close date passed, still unsubmitted
        totalCancelled: sql<number>`COUNT(CASE
          WHEN ${examSubmissions.submittedAt} IS NULL
            AND ${exams.endTime} <= NOW()
          THEN 1 END)`,
